...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        const { data } = await axios.get(`${BASE_URL}/api/analysis`);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const { data } = await axios.get(`${BASE_URL}/api/scans/stats`);
        setScanStats(prev => ({ ...prev, ...data }));
      } catch (err) {
        console.error("Failed to fetch scan stats:", err.message);
      }
    };
    fetchStats();
  }, [BASE_URL]);
  ...
  const confirmLogout = () => { setShowLogoutDialog(false); navigate("/login"); };
  const cancelLogout = () => setShowLogoutDialog(false);
  const openAdminDialog = () => setShowAdminDialog(true);
  const closeAdminDialog = () => setShowAdminDialog(false);
  const handleChangePassword = () => { setShowAdminDialog(false); navigate("/change-password"); };
  ...
  return (
    <div className="dashboard-container">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="logo-section" onClick={() => { setActiveNav("Dashboard"); setSidebarOpen(false); }} style={{ cursor: "pointer" }}>
                ...
                <div className="chart-section">
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={analysisData.predictions}>
                      ...
                      <Line type="monotone" dataKey="P" stroke="#ff4d4d" name="Pathologic (P)" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
